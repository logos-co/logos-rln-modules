...
//!   "sup3rsecure") only verifies under keccak256 — the keyfile
//!   construction the nwaku ecosystem actually writes. Compatibility with
//!   real keystores wins; pinned by `spec_test_vector_decrypts`.
...
// The envelope crypto and durable-file primitives live in the keystore-core
// subcrate; re-exported so `keystore::` stays the single path for keystore
// machinery.
pub(crate) use keystore_core::{
...
pub(crate) struct KeystoreFile {
    ...
    /// password). Generated at the unlock that initializes an EMPTY
    /// keystore; a non-empty file without it refuses to unlock.
...
/// tamper-bound by `allocations_mac`.
...
// ------------------------------------------------------------------- file IO
//
// Both delegate to keystore-core's durable-file layer: fail-closed
...
pub(crate) fn load(dir: &Path) -> io::Result<KeystoreFile> {
    keystore_core::load_json(dir, KEYSTORE_FILE)
}

pub(crate) fn save_atomic(dir: &Path, file: &KeystoreFile) -> io::Result<()> {
    keystore_core::write_durable_json(dir, KEYSTORE_FILE, file)
}
