//! The encrypted credential envelope: Ethereum-V3-keyfile construction
//! (PBKDF2-HMAC-SHA256 key derivation, AES-128-CTR over `dk[0..16]`,
//! `mac = keccak256(dk[16..32] ‖ ciphertext)`).
//!
//! NOTE the MAC hash: the WAKU-RLN-KEYSTORE prose says SHA256, but its own
//! test vector (password "sup3rsecure") only verifies under keccak256 — the
//! Ethereum-V3 / nim-eth keyfile construction the nwaku ecosystem actually
//! writes. Compatibility with real keystores wins; pinned by
//! `spec_test_vector_decrypts`. Reads honor each envelope's own `kdfparams`
//! (so foreign iteration counts decrypt); writes use `WRITE_KDF_ROUNDS`.
...
/// PBKDF2 rounds for envelopes this crate writes — the nwaku ecosystem
...
/// The ONE PBKDF2 site: every key this crate (or a consumer) derives from a
/// password goes through here, so KDF paths can never drift apart.
...
/// Encrypt one plaintext under `password` with fresh salt + IV.
