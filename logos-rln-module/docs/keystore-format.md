...
| `rln_autounlock.secret` | OPTIONAL, plaintext, 0600: the module-owned auto-unlock password (full-lazy custody default — self-provisioned on a fresh store unless `LOGOS_RLN_DISABLE_AUTO_UNLOCK=1`). Present ⇒ at-rest confidentiality reduces to filesystem ACLs; the counter ledger's authentication is unaffected. Absent ⇒ the store is user- or keychain-owned. Deleting it orphans auto-created credentials. A secret here that does not open the store is moved aside as `.bad.<unix-ts>` evidence (a stale file must never permanently shadow the keychain source); one that is present but cannot be READ (any I/O error, or empty) blocks self-provisioning and is named in the refusal, never silently written over | durable write (tmp → fsync → rename), written BEFORE the store adopts the password |
